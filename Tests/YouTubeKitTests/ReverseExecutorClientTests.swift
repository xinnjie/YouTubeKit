  ...
  struct ReverseExecutorClientTests {
    ...
    func extractReturnsRemoteStreams() async throws {
      ...
      let infoJson = """
        ...
        """
      let infoData = try #require(infoJson.data(using: .utf8))
      ...
      #expect(stream.itag == 18)
      #expect(stream.ext == "mp4")
      #expect(stream.videoCodec == "avc1")
      #expect(stream.audioCodec == "mp4a.40.2")
      #expect(stream.averageBitrate == 100_000)
      #expect(stream.audioBitrate == 64_000)
      #expect(stream.videoBitrate == 36_000)
      #expect(stream.filesize == 1234)

      let messages = await recorder.allMessages()
      #expect(messages.count >= 2)
      if case .hello(let hello) = messages.first?.payload {
        #expect(hello.deviceID == "device-id")
        #expect(hello.userAgent == "unit-test")
      } else {
        #expect(Bool(false))
      }
